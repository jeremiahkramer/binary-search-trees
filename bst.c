...
/*
 * This is a helper function for traversing the tree for the bst_size function
 * 
 * Params:
 *  n - bst_node that we are traversing
 *  i - val that we are incrementing by 1 to count nodes in tree
 */

void euler_tour_count(struct bst_node* n, int* i){ 

  //recursively call euler tour to traverse tree and count the number of nodes while traversing tree

  if(n){
    euler_tour_count(n->left, i);
    (*i)++;
    euler_tour_count(n->right, i);
  }
}
...
int bst_size(struct bst* bst) {
  int i = 0; //start at 0 nodes
  euler_tour_count(bst->root, &i);
  return i;
}

/*
 * This is a helper function for getting max height of a tree if we are passed a bst_node
 * 
 * Params:
 *  n - bst_node that we are traversing
 *
 * Return:
 *  max height of sub tree of the whole tree
 */

int max_height(struct bst_node* n){
  if(n == NULL){ //height is -1 if node is null
    return -1;
  }

  int l_height = max_height(n->left); //get the max height of left sub tree 
  int r_height = max_height(n->right); //get the max height of the right sub tree

  if(l_height > r_height)
    return l_height + 1; //if l sub tree is greater, return that val
  else
    return r_height + 1; //if r sub tree is greater, return that val
}
...
int bst_height(struct bst* bst) {
  return max_height(bst->root);
}

/*
 * This is a helper function for getting path sum of a tree fro root to leaf
 * it subtracts the sum we are searching for with the node it encounters
 * that's why if the val at the leaf is equal to this subtracted sum val, the sum path exists
 * 
 * Params:
 *  n - bst_node that we are traversing
...
int path_sum(struct bst_node* n, int sum){
  if(n == NULL) //base case
    return 0;
  if(n->left == NULL && n->right == NULL){ //if the node is a leaf
    if(n->val == sum) //if sum is equal to leaf node val
      return 1; //return true
    else
      return 0; //else return false
  }
  if(path_sum(n->left, sum - n->val)) //if it has a left child, check left sub tree with new sum (sum - val at node)
    return 1;
  if(path_sum(n->right, sum - n->val)) //if it has a right child, check right sub tree with new sum (sum - val at node)
    return 1;
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
  if(stack_isempty(iter->s) && iter->curr == NULL) //if the stack is empty and the curr node is equal to null, return 0 (no more nodes to visit)
    return 0;
  else
    return 1;
}
...
int bst_iterator_next(struct bst_iterator* iter) {
  ...
  while(1){ //we can do this because it will always be broken by the return statement
    if(iter->curr != NULL){ //if curr node is NULL
      stack_push(iter->s, iter->curr); //push on stack 
      iter->curr = iter->curr->left; //point to currs left child
    }
    else{
      iter->curr = stack_pop(iter->s); //pop the last node that was pushed on stack and set curr equal to this (traverses back up tree)
      i = iter->curr->val; //save the val in this node
      iter->curr = iter->curr->right; //point curr to right child
      return i; //return val at this node
    }
  }
